...
export async function fetchAvailableSkills() {
  ...
  try {
    ...
    throw new Error(getApiErrorMessage(error, "Failed to load skills"), { cause: error });
  }
...
export async function createSkill(payload) {
  ...
  try {
    const response = await apiClient.post('/admin/skills', {
      name: payload.name,
...
export async function updateSkill(skillId, payload) {
  ...
  try {
    const response = await apiClient.put(`/admin/skills/${skillId}`, {
      name: payload.name,
      category_id: payload.categoryId,
    })
    return { success: true, data: response.data }
