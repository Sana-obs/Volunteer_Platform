...
function attachComputedStatus(opportunity) {
  ...
  return {
    ...withLiveCount,
    status: getEffectiveOpportunityStatus(withLiveCount),
  }
}
...
function buildOpportunityFormData(payload, imageFile, governorates = []) {
  ...
  const governorate = getGovernorateBySelectValue(
  ...
  const skillIds = Array.isArray(payload.skills)
    ? payload.skills
        .map((skill) => (typeof skill === 'object' ? skill?.id : skill))
        .filter(Boolean)
    : []
  ...
  if (payload.registerStartAt) {
    formData.append('register_start_at', payload.registerStartAt)
  }

  if (payload.registerEndAt) {
    formData.append('register_end_at', payload.registerEndAt)
  }

  if (payload.minHours != null) {
    formData.append('min_hours', payload.minHours)
  }

  if (payload.maxHours != null) {
    formData.append('max_hours', payload.maxHours)
  }

  if (payload.totalHours != null) {
    formData.append('total_hours', payload.totalHours)
  }

  if (payload.minVolunteers != null) {
    formData.append('min_volunteers', payload.minVolunteers)
  }

  if (payload.maxVolunteers != null) {
    formData.append('max_volunteers', payload.maxVolunteers)
  }
...
export function matchesFilters(opportunity, filters = {}) {
  ...
  const opportunityCategoryId =
    normalizedOpportunity.category?.id != null
      ? String(normalizedOpportunity.category.id)
      : ''
  ...
  const normalizedCategoryIds = Array.isArray(categoryIds)
    ? categoryIds.filter(Boolean).map(String)
    : []

  const normalizedSkillIds = Array.isArray(skillIds)
    ? skillIds.filter(Boolean).map(String)
    : []
  ...
  const matchesSearch =
    !search ||
    opportunityTitle.toLowerCase().includes(search.trim().toLowerCase())

  const matchesCategory =
    (!normalizedCategoryId && normalizedCategoryIds.length === 0) ||
    opportunityCategoryId === normalizedCategoryId ||
    normalizedCategoryIds.includes(opportunityCategoryId)

  const matchesSkill =
    (!normalizedSkillId && normalizedSkillIds.length === 0) ||
    opportunitySkills.some((skill) => {
      const opportunitySkillId =
        skill?.id != null ? String(skill.id) : ''

      return (
        opportunitySkillId === normalizedSkillId ||
        normalizedSkillIds.includes(opportunitySkillId)
      )
    })
...
function computeMatchScore(opportunity, { skillIds, skillNames, city }) {
  ...
  const matchingSkillsCount = opportunitySkills.filter((skill) =>
    skillIds.includes(skill.id),
  ).length

  const matchingSkill = opportunitySkills.find((skill) =>
    skillIds.includes(skill.id),
  )
  ...
  if (matchingSkill) {
    reasons.push({
      ...
      text: `Matches your ${
        skillNames.get(matchingSkill.id) || matchingSkill.name
      } skill`,
    })
  }
...
export async function fetchCompletedOpportunities(governorates = []) {
  ...
  try {
    ...
    throw new Error(
      getApiErrorMessage(
        error,
        'Failed to load completed opportunities',
      ),
      { cause: error },
    )
  }
 ...
 */
export async function fetchSuggestedOpportunities(
  { skillIds = [], city = '' } = {},
  governorates = [],
) {
  if (MOCK_MODE) {
    ...
    return MOCK_OPPORTUNITIES
      ...
      .map(({ opportunity, reason }) => ({
        ...attachComputedStatus(opportunity),
        matchReason: reason,
      }))
  }
  ...
  try {
    const response = await apiClient.get(
      '/volunteers/me/suggested-opportunities',
    )

    // apiClient's response interceptor already unwraps the Laravel envelope,
    // so response.data is the opportunities array itself (same as the other
    // fetchers in this file).
    const data = Array.isArray(response.data) ? response.data : []

    return data
      .filter((item) => item.isSuitable === true)
      .map((item) => ({
    ...
    throw new Error(
      getApiErrorMessage(
        error,
        'Failed to load suggested opportunities',
      ),
      { cause: error },
    )
  }
...
export async function fetchOpportunityById(id, governorates = []) {
  if (MOCK_MODE) {
    ...
    const opportunity =
      MOCK_OPPORTUNITIES.find((item) => item.id === id) || null
    ...
    const similar = opportunity
      ? MOCK_OPPORTUNITIES
          .filter(
            (item) =>
              item.id !== id &&
              item.category?.id === opportunity.category?.id,
          )
          .slice(0, 3)
      : []
  ...
  try {
    ...
    const opportunity = mapApiOpportunity(
      response.data,
      governorates,
    )
    ...
    if (opportunity?.category?.id) {
      try {
        const sameCategory = await fetchOpportunities(
...
export async function fetchMyOpportunities(
  ...
  if (MOCK_MODE) {
    ...
    return MOCK_OPPORTUNITIES
      .filter(
        (opportunity) =>
          opportunity.organization?.id === MOCK_MY_ORGANIZATION_ID,
      )
      .map(attachComputedStatus)
  }
  ...
  return all.filter(
    (opportunity) =>
      String(opportunity.organization?.id) ===
 ...
 */
export async function fetchOpportunitiesByOrganization(
  organizationId,
  governorates = [],
) {
  if (MOCK_MODE) {
...
export async function deleteOpportunity(id) {
  if (MOCK_MODE) {
    ...
    const index = MOCK_OPPORTUNITIES.findIndex(
      (item) => item.id === id,
    )
...
export async function createOpportunity(
  ...
  const selectedGovernorate = selectedCityValue
    ? getGovernorateBySelectValue(
        selectedCityValue,
        governorates,
      )
  ...
  if (
    selectedGovernorate &&
    selectedGovernorate.isActive === false
  ) {
    return {
      success: false,
      error:
        'This governorate is no longer served by the platform.',
    }
  }
  ...
  if (MOCK_MODE) {
    ...
    const mockUser = email
      ? loadMockUsers().find((user) => user.email === email)
      : null
    ...
    return {
      success: true,
      data: attachComputedStatus(newOpportunity),
    }
  }
  ...
  if (MOCK_MODE) {
    ...
    if (index !== -1) {
      MOCK_OPPORTUNITIES[index] = {
        ...
        image: imageFile
          ? URL.createObjectURL(imageFile)
          : MOCK_OPPORTUNITIES[index].image,
      }
    }

    return {
      success: true,
      data: attachComputedStatus(MOCK_OPPORTUNITIES[index]),
    }
  }
 ...
 * @param {string} id
 * @param {number} committedHours
 */
export async function participateInOpportunity(
  id,
  committedHours,
) {
  if (MOCK_MODE) {
    ...
    const liveOpportunity = {
      ...opportunity,
      currentVolunteers: computeLiveCurrentVolunteers(
        opportunity.id,
      ),
    }

    if (
      getEffectiveOpportunityStatus(liveOpportunity) !==
      OPPORTUNITY_STATUS.REGISTRATION_OPEN
    ) {
      return {
        success: false,
        error:
          'Registration is no longer open for this opportunity',
      }
    }
    ...
    if (
      ...
      hours < opportunity.minHours ||
      hours > opportunity.maxHours
    ) {
      return {
        success: false,
    ...
    if (mockUser) {
      ...
      addMockParticipation({
        ...
        volunteerProfile: {
          ...
          educationLevel:
            mockUser.educationLevel || '',
          dateOfBirth:
            mockUser.dateOfBirth || null,
...
export async function setOpportunityStatus(
  ...
  if (MOCK_MODE) {
    ...
    const isClosing =
      status === OPPORTUNITY_STATUS.REGISTRATION_CLOSED

    MOCK_OPPORTUNITIES[index] = {
      ...MOCK_OPPORTUNITIES[index],
      registrationClosedManually: isClosing,
      registrationClosedReason: isClosing
        ? 'organization'
        : null,
    }
 ...
 */
export function closeCityOpportunitiesRegistration(
  cityNameEn,
) {
  ...
  MOCK_OPPORTUNITIES.forEach((opportunity) => {
    ...
    if (
      getEffectiveOpportunityStatus(opportunity) !==
      OPPORTUNITY_STATUS.REGISTRATION_OPEN
    ) {
      return
    }
    ...
    opportunity.registrationClosedReason =
      'city_deactivated'
  })
}