// blue/purple/green/red map to semantic tokens (info/achievement/success/danger).
const COLOR_STYLES = {
...
// Dark variants for the admin panel (same hues, dark-surface contrast).
...
export default function Chip({ children, color = "primary", dark = false, className = "", customStyle = "" }) {
  ...
  return (
    <span
      className={`inline-flex items-center whitespace-nowrap px-3 py-1 rounded-full text-sm border font-medium ${styles} ${className}`}
    >
      {children}
    </span>
  );
