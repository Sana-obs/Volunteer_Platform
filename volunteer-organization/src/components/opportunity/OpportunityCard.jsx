...
export default function OpportunityCard({
  ...
  // Strongest match reason only; hidden when it's shown as an external section heading.
  const matchReason = recommended && showMatchReason ? opportunity.matchReason : null;

  // Category-colored fallback when there's no photo (not uploaded, or backend-rejected).
  ...
  // Solid (not translucent) so it stays legible over any background image.
  const recommendedBadge = recommended ? (
    <span className="inline-flex items-center gap-1 rounded-full bg-primary text-white text-xs font-semibold px-3 py-1">
      <Target size={12} aria-hidden="true" /> Recommended
    </span>
  ) : null;
    ...
    >
      ...
      {(opportunity.category && showCategoryChip) || opportunity.isGroup === true ? (
        <div className="flex flex-wrap items-center gap-2 mb-4">
          ...
          {/* Explicit true only. The field also feeds a backend achievement check, so it's not UI-only. */}
