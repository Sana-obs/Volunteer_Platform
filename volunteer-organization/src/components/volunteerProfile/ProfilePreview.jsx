...
import {
  ...
  CATEGORY_COLORS,
  getCategoryLabel,
  getUniqueCategoryNamesForSkills,
...
export default function ProfilePreview({ fullName, email, phone, availableSkills }) {
  ...
  // Full skill objects, including the category object
  const selectedSkills = (values.skills || []).map((id) =>
    availableSkills.find((s) => s.id === id)
  );

  const interestCategories = getUniqueCategoryNamesForSkills(values.skills, availableSkills);
    ...
    <div className={`${PANEL_SURFACE} border-l-4 border-l-info p-6 md:p-8`}>
      ...
      <div className="space-y-4">
        ...
        <div className="flex justify-between items-start py-1 gap-3">
          <Typography variant="bodySm" color="muted" className="whitespace-nowrap shrink-0">
            Interests
          </Typography>

          <div className="min-w-0 flex-1 flex flex-wrap justify-end gap-1.5">
            {interestCategories.length ? (
              interestCategories.map((category) => {
                ...
                return (
                  <Chip
                    key={category}
                    customStyle={color}
                    className="bg-transparent! px-2! py-0.5! text-xs!"
                  >
              ...
              <Typography variant="bodySm" color="heading" weight="medium">
                —
              </Typography>
            )}
          </div>
