// Generic admin section (header + add button + list/loading/empty), reused for categories and skills.
...
export default function CatalogSection({
  ...
  items,
  isLoading,
  // optional — omitting it keeps the prior behavior (isError defaults to false)
  isError = false,
        ...
        <div className="flex flex-col gap-3">
          {Array.from({ length: 3 }).map((_, index) => (
            <div key={index} className={`${ADMIN_CARD_SURFACE} p-4`}>
              <Skeleton dark className="h-5 w-1/3 mb-2" />
              <Skeleton dark className="h-4 w-2/3" />
            </div>
          ))}
        </div>
      ) : isError ? (
        // Show a fetch failure explicitly with retry, not as an empty list.
